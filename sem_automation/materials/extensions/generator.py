# 模块：sem_automation/materials/extensions/generator.py；内部模块由统一入口调用。
...
def get_output_dir(project_name: str, project_root=None) -> Path:
...
def load_website_json(project_name: str, project_root=None) -> dict:
    path = get_output_dir(project_name, project_root) / "website_pages.json"

    if not path.exists():
        raise FileNotFoundError(
            f"找不到网页读取结果：{path}\n"
            f"请先运行 sem_automation.readers.web_reader。"
        )
...
def build_prompt(
    ...
    prompt = load_prompt("materials/sitelink_callout")
...
def build_callout_only_prompt(project_brief: str) -> str:
    prompt = load_prompt("materials/callout_only")
