# 模块：sem_automation/materials/context/collect.py；内部模块由统一入口调用。
...
from sem_automation.readers.file_reader import read_file
...
def collect_project_data(project_name: str, *, project_root: Path | None = None) -> dict:
    ...
    project_dir = material_dir("uploads", project_name, root)
    ...
    final_content = "\n".join(all_text)
    output_dir = material_dir("outputs", project_name, root)
    output_dir.mkdir(parents=True, exist_ok=True)

    raw_text_local_path = output_dir / "raw_text_local.txt"
    raw_text_local_path.write_text(final_content, encoding="utf-8")
    ...
    elif website_url_status == "preserved_explicit":
        print("\n已保留通过 --website 显式配置的客户网站。")
    else:
        print("\n客户资料中未识别到网站，本项目不生成 website_urls.txt。")

    print(f"\n本地资料文本已保存：{raw_text_local_path}")
    print(f"文件读取报告已保存：{report_path}")
    print(f"总共提取字符数：{len(final_content)}")
    print(f"共扫描文件数：{len(file_reports)}")

    generated_files = [raw_text_local_path, report_path]
    ...
    return {
        ...
        "output_dir": output_dir,
        "raw_text_local_path": raw_text_local_path,
