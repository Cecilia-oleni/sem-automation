...
# & ".\.venv\Scripts\python.exe" -m sem_automation.materials.negatives.generator
# 不要使用右上角的播放按钮来启动代码，因为会找不到 modules 包

from sem_automation.ai.task_runner import run_ai_task
from sem_automation.materials.ads.generator import group_keywords, load_keyword_v2
...
def load_keyword_reference(project_name, output_dir, keyword_version, project_root=None):
    ...
    if keyword_version in ["auto", "keyword_v2", "keywords_v2"]:
        try:
            return format_keyword_v2_for_prompt(project_name, project_root), "keyword_v2.xlsx / keywords_v2.xlsx"
...
def generate_negative_keywords(
    ...
    return run_ai_task(
        output_dir=output_dir,
        project_name=project_name,
        prompt_name="materials/negative_keyword",
