...
# & ".\.venv\Scripts\python.exe" -m sem_automation.materials.ads.generator
...
def get_output_dir(project_name, project_root=None):
    return material_dir("outputs", project_name, Path(project_root or get_project_root()))
...
def load_project_brief(project_name, project_root=None):
    path = get_output_dir(project_name, project_root) / "project_brief.md"
    return read_text(path, "项目资料分析 project_brief.md")

def load_negative_keywords(project_name, project_root=None):
    path = get_output_dir(project_name, project_root) / "negative_keywords.md"
...
def build_prompt(project_brief, negative_keywords, campaign, adgroup, keywords):
    prompt = load_prompt("materials/ad_copy")
