...
from sem_automation.materials.extensions.generator import (
...
class WebsiteUrlExtractorTests(unittest.TestCase):
    ...
    def test_outputs_path_precedes_legacy_uploads_path(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            output_path = root / "outputs" / "materials" / "项目A" / "website_urls.txt"
            legacy_path = root / "uploads" / "materials" / "项目A" / "website_urls.txt"
...
class WebsiteWorkflowModeTests(unittest.TestCase):
    def test_web_reader_skips_project_without_website(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch("sem_automation.materials.context.crawl.get_project_root", return_value=Path(temp_dir)):
    ...
    def test_website_config_without_crawl_result_errors_before_api(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch(
                "sem_automation.materials.extensions.generator.get_project_root",
                return_value=root,
            ):
                with self.assertRaisesRegex(FileNotFoundError, "请先运行 sem_automation.readers.web_reader"):
                    generate_sitelink_callouts("项目A")

    def test_callout_only_generation_writes_all_three_outputs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            output_dir = root / "outputs" / "materials" / "无网站项目"
            output_dir.mkdir(parents=True)
            ...
            with patch(
                ...
                "sem_automation.materials.extensions.generator.call_llm",
